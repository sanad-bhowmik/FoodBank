        ...
        <div class="row align-items-center">
            <div class="col-12 col-md-7 col-lg-6">
                ...
                <form method="GET" action="{{ route('search') }}" id="myForm">
                    <div class="main-search-input">
                        ...
                        <!--                <path d="M12 19.5C16.1421 19.5 19.5 16.1421 19.5 12C19.5 7.85786 16.1421 4.5 12 4.5C7.85786 4.5 4.5 7.85786 4.5 12C4.5 16.1421 7.85786 19.5 12 19.5Z" stroke="#EE1D48" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />-->
                        <!--                <path d="M12 15C13.6569 15 15 13.6569 15 12C15 10.3431 13.6569 9 12 9C10.3431 9 9 10.3431 9 12C9 13.6569 10.3431 15 12 15Z" stroke="#EE1D48" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />-->
            ...
            <div class="col-12 col-md-5 col-lg-6">
                <div class="banner-image">
                    <img src="{{ asset('/public/images/' . setting('banner_image')) }}" alt="hero">
                  
                    <!--<img src="https://img.freepik.com/premium-photo/nutritious-breakfast-with-diverse-options-showcased-against-white-background-ai_894067-634.jpg" alt="hero">-->
...
<!--======== ITEM PART END ========-->
<section class="item" style="padding-bottom: 60px;width: 78%;margin-left: 46px;">
    <div class="" style="width: 120%;">
        <h2 class="section-title">{{ __('Our Popular Items') }}</h2>
        <span class="section-subtitle"> {{ __('Taste Great') }} </span>
        <div style="display: flex;">
            <?php
            $menuItems = \App\Models\MenuItem::take(5)->get();
            ?>

            @foreach ($menuItems as $menuItem)
                <div class="card" style="height: 50%; margin-right: 10px; cursor: pointer;" id="hover" data-bs-toggle="modal" data-bs-target="#shop-modal"
                    data-item-id="{{ $menuItem->id }}" data-item-name="{{ $menuItem->name }}"
                    data-item-image="{{ $menuItem->getImageAttribute() }}" data-item-description="{{ $menuItem->description }}" data-unit-price="{{ $menuItem->unit_price }}" data-discount-price="{{ $menuItem->discount_price }}"
                    style="cursor: pointer;">
                    <img src="{{ $menuItem->getImageAttribute() }}" alt="Item Image">
                    <h3 class="card-title">{{ $menuItem->name }}</h3>
                    <button type="button" style="width: 80px;margin-left: 26%;background-color: #EE1D48;color: white;border-radius: 10px;width: 48%;font-family: &quot;OpenSauceOne&quot;, sans-serif;font-size: 14px;font-weight: 500;">
                        <i class="fa-solid fa-eye" style="color: #fff;"></i>Details
                    </button>
                </div>
            @endforeach
...
<section class="app section-gap-90">
    <div class="container">
        <div class="row">
            ...
            <div class="col-12 col-sm-6">
                <div class="app-image">
                    <img src="{{ asset('frontend/images/mockup.png') }}" alt="mockup">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-6 col-md-4 col-lg-3">
                <a href="{{ route('search', ['cuisines' => [$bestSellingCusine->slug], 'expedition' => 'all']) }}" class="category-card">
                    <img class="bestSellingCusineImage" src="{{ $bestSellingCusine->image }}" alt="category">
...
<section class="restaurant section-gap-66">
    <div class="container" >
        <h2 class="section-title borderd">{{ __('More Items') }}</h2>
        <div class="row">
            @foreach ($bestSellingRestaurants as $restaurant)
            <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                <a href="{{ route('restaurant.show', [$restaurant]) }}" class="restaurant-card"style="box-shadow: rgba(0, 0, 0, 0.25) 0px 14px 28px, rgba(0, 0, 0, 0.22) 0px 10px 10px;">
                    <figure class="figure position-relative">
                        <img class="bestSellingRestaurantsImage" src="{{ $restaurant->image }}" alt="restaurant">
                    ...
                    <div class="content">
                        ...
                        <div class="location">
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M7.99992 8.95346C9.14867 8.95346 10.0799 8.02221 10.0799 6.87346C10.0799 5.7247 9.14867 4.79346 7.99992 4.79346C6.85117 4.79346 5.91992 5.7247 5.91992 6.87346C5.91992 8.02221 6.85117 8.95346 7.99992 8.95346Z" stroke="#1F1F39" stroke-width="1.5" />
                                <path d="M2.4133 5.66016C3.72664 -0.113169 12.28 -0.106502 13.5866 5.66683C14.3533 9.0535 12.2466 11.9202 10.4 13.6935C9.05997 14.9868 6.93997 14.9868 5.5933 13.6935C3.7533 11.9202 1.64664 9.04683 2.4133 5.66016Z" stroke="#1F1F39" stroke-width="1.5" />
                            </svg>
...
<!-- Shop Modal -->
<div class="modal fade" id="shop-modal" tabindex="-1" aria-labelledby="shop-modal-label" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="shop-modal-label">{{ $menuItem->name }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <img src="{{ $menuItem->getImageAttribute() }}" alt="Item Image" style="width: 100%;">
                <h3 class="card-title">{{ $menuItem->name }}</h3>
            ...
            <button type="button" class="rest-book-btn " data-bs-toggle="modal" data-bs-target="#shop-modal"style=" margin-left: 46%; margin-top: 24px;font-size: 14px;">
                <i class="fa-solid fa-shopping-bag" style="color: #fff;"></i><span>Add Cart</span>
            </button>
            </div>
        </div>
    </div>
...
@push('js')
<!-- <script src="https://maps.googleapis.com/maps/api/js?key={{ setting('AIzaSyC52koocV79jA34n9nHZHxyAcLVaQhWkXQ') }}&sensor=false&libraries=places&callback=initAutocomplete">AIzaSyC52koocV79jA34n9nHZHxyAcLVaQhWkXQ -->
...
<script>
    $(document).ready(function() {
        $('.rest-book-btn').on('click', function() {
             window.location.href = 'restaurant/fs-bd'
        });
    });
</script>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $('#shop-modal').on('show.bs.modal', function (event) {
            ...
            var unitPrice = parseFloat(button.data('unit-price')).toFixed(2); // Format to two decimal places
            var discountPrice = parseFloat(button.data('discount-price')).toFixed(2); // Format to two decimal places
            ...
            var modalBodyContent = `
                <img src="${itemImage}" alt="${itemName}" style="max-width: 100%;">
                <p style="text-align: center;">${itemDescription}</p>
                <div style="display: flex;margin-top: 21px;font-size: 22px;margin-left: 139px;"">
                    <p>Price:৳${unitPrice.replace(/\.00$/, '')}</p>
            `;

            // Check if there is a discount price and it's not zero
            if (discountPrice && discountPrice !== "0.00") {
                modalBodyContent += `<p style="text-decoration: line-through;margin-left: 7px;color: var(--text);padding: 0 4px;border-radius: 6px;">৳${discountPrice.replace(/\.00$/, '')}</p>`;
            }
...
<script>
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                function(position) {
                    ...
                    // Update the autocomplete input with the user's location
                    var locationText =  lat + ', ' + long;
                    updateLocationInput(locationText);

                    // You might want to store lat and long in hidden fields for further use
    ...
    function updateLocationInput(location) {
        ...
        if (autocompleteInput) {
            autocompleteInput.value = location;
        }
    }
</script>
<script>

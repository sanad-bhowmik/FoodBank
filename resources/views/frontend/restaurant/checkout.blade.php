            ...
            <form id="payment-form" action="{{ route('checkout.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="checkout-group">
                    <div class="checkout-delivery">
                                        ...
                                        @foreach ($addresses as $address)
                                            @if ($address->id != (isset($lastAddress) ? $lastAddress->id : 0))
                                                <div class="moreAddress hide">
                                                    <label class="checkout-label" for="work">
                                                        <input type="radio" class="form-radio"
                                                            onChange="deliveryAddress({{ $address->latitude }},{{ $address->longitude }})"
                                                            value="{{ $address->id }}" name="address" id="work">
                                                        <dl>
                        ...
                        <div class="checkout-card mb-0" style="box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;">
                            <div class="checkout-card-head" id="addressSection">
    <h3>{{ __('Address') }}:</h3>
    <textarea id="1address" name="1address" placeholder="{{ __('Full Address') }}" style="border: 1px solid var(--border-gray); padding: 10px; text-align: center; border-radius: 10px;" required>{{ old('address', auth()->user()->address) }}</textarea>
    
</div>

<div class="checkout-card-head">
    <h3>{{ __('Instruction') }}:</h3>
    <textarea id="instruction" name="instruction" placeholder="{{ __('Instruction') }}" style="border: 1px solid var(--border-gray); padding: 10px; text-align: center; border-radius: 10px;">{{ old('instruction') }}</textarea>
</div>
...
<div class="text-danger">
    ...
    @enderror
       ...
       type="tel" id="" name="mobile" onkeypress='validate(event)' required
       value="{{ old('mobile', auth()->user()->phone) }}">


                                <input type="hidden" id="code" name="countrycode" value="+88">
                    ...
                    <div class="checkout-summary" style="box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="addressForm" method="post" >
                    ...
                    <div class="address-modal-search justify-content-between">
                        ...
                        <a href="javascript:void(0)">
                            <button id="locationIcon" onclick="getLocation()" class="lni lni-target"></button>
                        </a>
                    </div>
                    <div class="">
                    ...
                    <button data-bs-dismiss="modal" class="form-btn"
...
// <script>
//     // Function to get the order status (replace this with your actual implementation)
//     function getOrderStatus() {
//         // Replace this with your logic to get the order status
//         return 'pickup'; // Change this value based on your actual order status
//     }

//     // Check order status on page load
//     window.onload = function () {
//         toggleAddressTextarea();
//     };

//     // Function to toggle address textarea based on order status
//     function toggleAddressTextarea() {
//         var orderStatus = getOrderStatus();
//         var addressSection = document.getElementById('addressSection');

//         // Check if the order status is 'pickup' and hide/show the address textarea accordingly

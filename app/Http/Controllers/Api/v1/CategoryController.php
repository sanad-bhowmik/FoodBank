...
class CategoryController extends BackendController
{
     ...
     public function store(CategoryRequest $request)
    {
        try {
            ...
            return $this->successResponse(['status' => 201, 'data' => $categoryResource], 'Category created successfully.');
        } catch (\Exception $e) {
            return $this->errorResponse('Error creating category.', 500, $e->getMessage());
        }
    }
    /**
    ...
    public function shows($id)
    {
        ...
        if (!blank($categories->MenuItems)) {
            foreach ($categories->MenuItems as $menuItem) {
                if (!array_key_exists($menuItem->menu_item_id, $restaurants)) {
                    $restaurants[$menuItem->menu_item_id] = $menuItem->restaurant;
                }
            }
        }
        try {
            $this->data['category'] = new CategoryResource($categories);
              $this->data['restaurants'] = MenuItemResource::collection($restaurants);

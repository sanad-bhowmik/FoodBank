...
class NotificationOrderController extends Controller
{
    ...
    public function index()
    {
        ...
        $orders = Order::where(['delivery_boy_id' => null,'order_type' => OrderTypeStatus::DELIVERY,])->whereIn('status', [ OrderStatus::PROCESS])->latest()->whereNotIn('id', $deliveryStatusHistoriesArray)->get();
        
        // $orders = Order::where(['delivery_boy_id' => null,'order_type' => OrderTypeStatus::DELIVERY,])->whereIn('status', [ OrderStatus::ACCEPT])->latest()->whereNotIn('id', $deliveryStatusHistoriesArray)->get();
...
//         $orders = Order::where(['delivery_boy_id' => null,'order_type' => OrderTypeStatus::DELIVERY,])->whereIn('status', [ OrderStatus::ON_THE_WAY])->latest()->whereNotIn('id', $deliveryStatusHistoriesArray)->get();
...
//     }
    ...
    public function getLimitAmount()
    {
        ...
        $acceptOrder        = Order::where('delivery_boy_id', auth()->id())->where('order_type',OrderTypeStatus::DELIVERY)->where('status', '!=', OrderStatus::COMPLETED)->sum('total');
